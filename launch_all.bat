...
if /i "%ADD_HAZARD%"=="y" (
    ...
    goto AUGMENT_FROM_CHECK
)

echo.
echo  Opening bounding box viewer for: %DATASET_DIR%
echo  (Press any key to advance through images, ESC or Q to quit)
echo.
python scripts/run_on_test_images.py --source "%DATASET_DIR%" --conf 0.3 --name dataset_check
echo.
echo  Annotated images saved to: runs/detect/dataset_check/
start "" "runs\detect\dataset_check"
echo.
goto MAIN_MENU

:AUGMENT_FROM_CHECK
set "FOLDER_CHOICE_SAVED=%FOLDER_CHOICE%"
...
if not defined TARGET_PER_CLASS (
    echo  Invalid selection.
    goto MAIN_MENU
)
...
REM Calculate copies needed: use a Python one-liner to determine the right --copies value
python -c "import os, math; imgs=len([f for f in os.listdir('%AUGMENT_TARGET%/train/images') if f.lower().endswith(('.jpg','.jpeg','.png'))]) if os.path.exists('%AUGMENT_TARGET%/train/images') else 30; copies=max(1, math.ceil(%TARGET_PER_CLASS%/max(1,imgs))-1); print(copies)" > _copies_tmp.txt 2>nul
set /p COPIES=<_copies_tmp.txt
del _copies_tmp.txt 2>nul

if not defined COPIES set COPIES=3
...
if /i "%INSERT_HAZARD%"=="y" (
    ...
    echo.
    echo  Hazard injection complete. Output: %AUGMENT_TARGET%_augmented_hazards/
)
...
echo     3) roboflow data
echo.
set /p TRAIN_FOLDER="  Select folder (1-3): "

if "%TRAIN_FOLDER%"=="1" set "TRAIN_DATA=image_data_normal/data.yaml"
if "%TRAIN_FOLDER%"=="2" set "TRAIN_DATA=image_data_with_synth/data.yaml"
if "%TRAIN_FOLDER%"=="3" set "TRAIN_DATA=roboflow data/data.yaml"

if not defined TRAIN_DATA (
    echo  Invalid selection.
    goto MAIN_MENU
)
...
echo     1) Random image from dataset (image_data_with_synth / roboflow data)
