...
This script trains the complete Architecture B pipeline:
- CNN Feature Extractor (shared with Architecture A)
- SpyNet optical flow network
- Pose Estimator (shared with Architecture A)

Target Performance:
- Training VRAM: ≤6GB (more efficient than Architecture A)
- Inference VRAM: ≤3GB
- Training time: ≤24 hours on consumer GPU
- Faster inference than Architecture A
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train Architecture B (SpyNet)'
    )
    ...
    parser.add_argument(
        '--checkpoint-dir',
        type=str,
        default='checkpoints/architecture_b',
...
def main():
    ...
    print("=" * 80)
    print("Architecture B (SpyNet) Training")
    print("=" * 80)
    ...
    hardware_ok, hardware_info = check_hardware_requirements(
        min_vram_gb=6.0,  # SpyNet requires less VRAM than LiteFlowNet2
        min_ram_gb=16.0,
    ...
    if not hardware_ok:
        ...
        logger.error("Architecture B requires:")
        logger.error("  - 6GB+ VRAM for training (less than Architecture A)")
    ...
    # SpyNet can handle larger batches with same VRAM
    if args.batch_size is None:
        vram_gb = hardware_info.get('vram_total_gb', 6.0)
        if vram_gb <= 6:
            args.batch_size = 3  # SpyNet is more efficient
        elif vram_gb <= 8:
            args.batch_size = 4
        elif vram_gb <= 16:
            args.batch_size = 6
    ...
    # Create models
    logger.info("Initializing Architecture B models...")
    
    feature_extractor = CNNFeatureExtractor(input_channels=3)
    flow_network = SpyNet()  # SpyNet instead of LiteFlowNet2
    ...
    logger.info(f"  SpyNet: {sum(p.numel() for p in flow_network.parameters()):,} parameters")
    logger.info(f"  Pose Estimator: {sum(p.numel() for p in pose_estimator.parameters()):,} parameters")
    
    # Training configuration (identical to Architecture A for fair comparison)
    config = {
        'architecture': 'Architecture_B_SpyNet',
        'target_resolution': tuple(args.target_resolution),
        'batch_size': args.batch_size,
        ...
        'run_name': 'architecture_b_spynet',
    ...
    logger.info("")
    logger.info("Architecture B advantages:")
    logger.info("  - Lower VRAM usage (6GB vs 8GB for training)")
    logger.info("  - Faster inference (~30ms vs ~50ms)")
    logger.info("  - More efficient for deployment")
    logger.info("")
